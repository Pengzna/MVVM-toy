...
const fragment_compile = (node, vm) => {
    /* 1. 插值表达式 {{}} */
    if (node.nodeType === 3) {
        const result_regex = regex_1.pattern_insertion.exec(node.nodeValue);
        if (result_regex) {
            ...
            if (result_regex) {
                ...
                node.nodeValue = node.nodeValue.replace(regex_1.pattern_insertion, value);
                new watcher_1.Watcher(vm, result_regex[1], (newValue) => {
                    node.nodeValue = init_insertion_expr.replace(regex_1.pattern_insertion, newValue);
                });
            }
        }
        return;
    ...
    if (node.nodeType === 1 && node.nodeName === 'INPUT') {
        ...
        attr.forEach((item) => {
            if (item.nodeName === 'v-model') {
                ...
                node.value = value;
                new watcher_1.Watcher(vm, item.nodeValue, (newValue) => {
                    node.value = newValue;
                });
