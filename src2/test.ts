...
//   //数据劫持 —— 监听实例里的数据
//   function Observer(data_instance: any) {
//     // 递归出口
...
//   function Complie(element: any, vm: any) {
...
//           new Watcher(vm, result_regex[1], (newVal: any) => {
...
//               .split(".")
//               .reduce((total: any, current: string | number) => total[current], vm.$data);
//             node.value = value;
//             new Watcher(vm, item.nodeValue, (newVal: any) => {
//               node.value = newVal;
//             });
...
//                 (total: any, current: string | number) => total[current],
//                 vm.$data
//               );
...
//       }
//       // 对子节点的所有子节点也进行替换内容操作
//       node.childNodes.forEach((child: any) => fragment_compile(child));
//     }
//     // 操作完成后将文档碎片添加到页面
